...
export default function PlateCard({ dish, index, onOpen }) {
  ...
  return (
    <article
      className={`group relative cursor-pointer overflow-hidden rounded-3xl border-2 border-ink bg-ink opacity-0 translate-y-6 animate-rise ${edge} transition-[border-color,box-shadow,transform] duration-300 ease-in-out hover:border-brass hover:shadow-[0_22px_50px_-20px_rgba(0,0,0,0.6)] focus-visible:outline-2 focus-visible:outline-brass focus-visible:outline-offset-4 motion-reduce:animate-none motion-reduce:opacity-100 motion-reduce:translate-y-0`}
      tabIndex={0}
      role="button"
    ...
    >
      ...
      />
      <div className={`border-t border-ink ${edge} px-5.5 pt-5 pb-5.5`}>
        <span className="mb-2 block font-mono text-[10.5px] tracking-[0.14em] text-paper-dim uppercase">
          Dish No. {dish.no}
        </span>
